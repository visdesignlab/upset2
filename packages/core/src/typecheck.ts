...
export function isUpsetConfig(config: unknown): config is UpsetConfig {
  ...
  if (!(
    ...
    && Object.hasOwn(config, 'activeSelection')
  ...
  const {
    ...
    plots, allSets, rowSelection, vegaSelection, querySelection, activeSelection, version, userAltText, intersectionSizeLabels, setSizeLabels,
  ...
  // activeSelection
  if (activeSelection !== null && !['row', 'vega', 'query'].includes(activeSelection)) {
