...
export const AltTextSidebar: FC<Props> = ({ open, close, generateAltText }) => {
  ...
  const [userLongText, setUserLongText] = useState(
    currState.userAltText?.longDescription,
  );
  const [userShortText, setUserShortText] = useState(
    currState.userAltText?.shortDescription,
  );
