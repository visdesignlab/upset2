...
import {
  showHiddenSetsSelector,
  showIntersectionSizesSelector,
  showSetSizesSelector,
} from '../atoms/config/displayAtoms';
...
export const SettingsSidebar = () => {
  ...
  return (
    <Box marginTop={collapsed ? '8px' : undefined}>
      <Box
        ...
        style={{
          transition: `all ease-out, border-radius .3s, ${
            collapsed ? 'max-height .2s, width .1s .2s' : 'max-height .2s .1s, width .1s'
          }`,
        }}
      >
        {collapsed ? (
          ...
          >
            <IconButton
              ...
              style={{ marginRight: BUTTON_PAD_LEFT }}
            >
              <MenuOpenIcon />
            </IconButton>
            Settings
        ...
        >
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography css={SIDEBAR_HEADER_CSS} variant="h2">
              General
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
                ...
                {aggregateByList.map((agg) => (
                  <Fragment key={agg}>
                    ...
                    >
                      ...
                      />
                      {agg !== 'None' && (
                        <HelpCircle key={`${agg} 1st`} text={helpText.aggregation[agg]} />
                      )}
                    </Box>
                    ...
                    .map((agg) => (
                      <Fragment key={agg}>
                        <Box
                          ...
                          key={agg}
                          aria-label={
                            agg !== 'None' ? helpText.aggregation[agg] : 'No aggregation'
                          }
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                ...
                <Slider
                  ...
                  onChange={(_, newVal: number | number[]) => {
                    if (typeof newVal === 'number') {
                      // if the sliders are set to the same value
