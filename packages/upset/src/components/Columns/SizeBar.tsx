...
export const SizeBar: FC<Props> = ({ row, size, vegaSelected, querySelected }) => {
  ...
  const { fullBars: fullSizeBars, remainder: sizeRemainder } = useMemo(
    () => calculateBars(size),
    [size, calculateBars],
  );
  const { fullBars: fullVegaBars, remainder: vegaRemainder } = useMemo(
    () => calculateBars(vegaSelected),
    [vegaSelected, calculateBars],
  );
  const { fullBars: fullQueryBars, remainder: queryRemainder } = useMemo(
    () => calculateBars(querySelected),
    [querySelected, calculateBars],
  );

  /** X-coord for the end of the selected bar */
  const vegaWidth = useMemo(
    () => calculateWidth(vegaSelected, vegaRemainder),
    [vegaRemainder, calculateWidth, vegaSelected],
  );

  /** X-coord for the end of the query bar */
  const queryWidth = useMemo(
    () => calculateWidth(querySelected, queryRemainder),
    [queryRemainder, calculateWidth, querySelected],
  );

  /** X-coord for the end of the size bar */
  const sizeWidth = useMemo(
    () => calculateWidth(size, sizeRemainder),
    [sizeRemainder, calculateWidth, size],
  );
    ...
    for (let i = 0; i < 3; ++i) {
      ...
      if (i < fullSizeBars) {
        result.push({
          ...
          fillColor:
            i < fullSelectBars ? darkenColor(i, selectionColor) : getFillColor(i),
          opacity: i < fullSelectBars ? SELECTION_OPACITY : 1,
          key: `$fullbar-${i}`,
        });
      } else if (i === fullSizeBars) {
