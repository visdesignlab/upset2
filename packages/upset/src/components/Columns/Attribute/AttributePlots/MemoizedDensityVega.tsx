...
export const MemoizedDensityVega: FC<Props> = memo(
  ({
    ...
    const spec = useMemo(
      () => generateAttributePlotSpec(AttributePlotType.DensityPlot, values, min, max, fillColor) ?? {},
      ...
      && (
        prevProps.values.length > 0
          ? prevProps.values[0] === nextProps.values[0]
            && prevProps.values[prevProps.values.length - 1] === nextProps.values[nextProps.values.length - 1]
          : true
      )
